...
$.ajax({
        ...
        success: function(res){
            ...
            if(res.code =='000000'){ 
                ...
                '<td style="width: 40px;color:#36f8ff;">单位名称</td>'+
                '<td style="width: 20px;color:#36f8ff;">项目数</td></tr>';
                ...
                if(type==4){
                    pieOption_4_5.series[0].name = "审计项目数";
                }else if(type==5){
                    pieOption_4_5.series[0].name = "审计覆盖情况";
                }
...
var lineOption_1 = {
        color: ['#3398DB','#04d20c','#090ad2'],
        tooltip: {
            trigger: 'axis',
            axisPointer: { // 坐标轴指示器，坐标轴触发有效
                type: 'shadow',// 默认为直线，可选为：'line' | 'shadow'
            },
		formatter: '{b0}<br/>{a0}:{c0}个<br/>{a1}:{c1}%<br/>{a2}:{c2}'
        },
        legend: {
            data: [],
            textStyle: {
        ...
        success: function(res){
            if (res.code == '000000') {
                ...
                html += '<tr class="liranking-th">'+
                    ...
                    '<td style="width: 10%;color:#36f8ff;">责任单位</td>'+
                    '<td style="width: 5%;color:#36f8ff;">计划完成时间</td>'+
                    '<td style="width: 6%;color:#36f8ff;">审计类型</td>'+
                    '<td style="width: 5%;color:#36f8ff;">责任人</td>'+
                    ' <td style="width: 5%;color:#36f8ff;">责任部门</td>'+
                    '<td style="width: 5%;color:#36f8ff;">问题金额</td>'+
                    '<td style="width: 15%;color:#36f8ff;">整改进展情况</td>'+
                    '<td style="width: 38%;color:#36f8ff;">问题描述</td> </tr>'
                $.each(data, function(commentIndex, item){
                    html += '<tr><td style="width: 1%;">' + (commentIndex + 1) + '</td>' +
                        '<td style="width: 10%;">' + item.susejdw + '</td>'+
                        '<td style="width: 10%;">' + item.beisjdw + '</td>'+
                        '<td style="width: 5%;">' + gettime(item.jihwcsj) + '</td>'+
                        '<td style="width: 6%;">' + item.shenjxmlx + '</td>'+
                        '<td style="width: 5%;">' + item.zhenggzrr + '</td>'+
                        '<td style="width: 5%;">' + item.zhenggzrbm + '</td>'+
                        '<td style="width: 5%;">' + item.wentje + '</td>'+
                        '<td style="width: 15%;">' + item.yuantzgjz+ '</td>'+
                        '<td style="width: 38%;">' + item.wentms + '</td></tr>';
                    });
