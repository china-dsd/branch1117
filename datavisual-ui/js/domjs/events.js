...
function getBarData_1(param = null) {
    $.ajax({
        type:"GET",
        url: url+"/fengxsj/barChart?type=2"+"&token="+token,
        data:param,
        dataType:"json",
        success: function(res){
            var data = res.data;
...
function select(name){
    $.ajax({
        type: 'GET',
        url: url+"/fengxsj/tableselect?token="+token,
        data: {"name":name},
        dataType:'json',
        success:function (res) {
            var data=res.data;//List集合
            ...
            if(money>=100000000){
                ...
                var num = money/100000000;
                var moneyy = num.toFixed(2);
                $("#sum").html(moneyy);
            }else  if(money>=10000){
                //$("#z5").html("亿元");
                document.getElementById('z5').innerText="亿元";
                var num = money/10000;
                var mon = num.toFixed(2);

                $("#sum").html(mon);
            }else {
                document.getElementById('z5').innerText="万元";
                $("#sum").html(money);
            }
            //document.getElementById('sum').innerText=money;

            $('#amount').html(eventSum);//绑定数量
            //document.getElementById('amount').innerText=eventSum;

            var area=document.getElementById("select");
            if(res.code =="000000"){
               // $.each(res,function(index,item){
                //     if(item[index]=="Erjdwmc"){
                //          Erjdwmc =item[index];
                //     }
                //     if(item[index]=="eventSum"){
                //         eventSum=item[index];
                //     }
                //     if(item[index]=="money"){
                //         money=item[index];
                ...
                if(Erjdwmc.length>0){
                   for(var i=0;i<Erjdwmc.length;i++){
                        optionstring += "<option value=" + Erjdwmc[i]+">"+Erjdwmc[i]+"</option>"; //公共变量赋值
                   }
