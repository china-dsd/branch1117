...
public class ShenjfxwtbajzggzqkbAPI {
     ...
     * 审计项目信息
     *
     * @param type 1审计问题整改迟缓事项 2表示审计问题金额
     * @return
     */
    ...
    public RestResult<List<SumModel>> pieChart(
            @ApiParam(value = "1表示审计项目类型, 2表示问题类型，3表示审计项目计划完成情况，4审计项目数，5审计覆盖情况")
            @RequestParam(defaultValue = "1") Integer type,
    ...
    public RestResult wordCloudChart(
        ...
        switch (type) {
            // 重大审计问题
            case 1:
                result.setData(shenjfxwtbajzggzqkbService.getSignificantShenjiEvent());
                break;
        }
        return result;
    }
