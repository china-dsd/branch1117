...
public class ZhongdjcfxpgtjServiceImpl extends AbsBaseServiceImpl implements ZhongdjcfxpgtjService {
  ...
  //根据名称进行模糊查询
    public List<Zhongdjcfxpgtj> selectbyname(String name) {
        ZhongdjcfxpgtjExample zExample = new ZhongdjcfxpgtjExample();
        zExample.createCriteria().andErjdwmcLike("%"+name+"%");
        List<Zhongdjcfxpgtj> result=zhongdjcfxpgtjMapper.selectByExample(zExample);
        return result;
    }
    // 事项风险程度排名
    @Override
    ...
    public List<LineChartModel> getSumAmountBarChart() {
        ...
        lowLineChartModel.setName(new ArrayList<>(keySet));
        ...
        middleLineChartModel.setName(new ArrayList<>(keySet));
        ...
        highLineChartModel.setName(new ArrayList<>(keySet));
        ...
        middleLineChartModel.setTitle("中");
        highLineChartModel.setTitle("高");
        String str[] = {"2018-06","2018-07","2018-08","2018-09","2018-10"};
        List<Integer> highList = new ArrayList<>();
        List<Integer> middleList = new ArrayList<>();
        List<Integer> lowList = new ArrayList<>();
        List<String> nameList = new ArrayList<>();
        ...
        for (int i = 0; i < str.length; i++) {
            lowList.add(random.nextInt(20));
            middleList.add(random.nextInt(20));
            highList.add(random.nextInt(20));
            nameList.add(str[i]);
        }
...
//            }
            //对数据进行规整分类
            String zuizsydwjc=item.getZuizsydwjc();
            if(zuizsydwjc==null)
                continue;
            if (zuizsydwjc.contains("一")){
                zuizsydwjc="一级单位";
            }else if (zuizsydwjc.contains("二")){
                zuizsydwjc="二级单位";
            }else if (zuizsydwjc.contains("三")){
                zuizsydwjc="三级单位";
            }else{
                zuizsydwjc="其他单位";
            }
            if (sumMap.containsKey(zuizsydwjc)) {
                sumMap.put(zuizsydwjc, sumMap.get(zuizsydwjc) + 1);
            } else {
                sumMap.put(zuizsydwjc, 1);
            }
        }
    ...
    public List<SumModel> getSumCountByAdoptionsed(String sort, boolean isYes) {
        Date endDate = ZXFKUtils.nowDate();
        Date startDate = ZXFKUtils.calcDateByNowDate(endDate, 0, -12, 0);
        List<Zhongdjcfxpgtj> dataList = findByDate(startDate, endDate);
        Map<String, Integer> sumMap = new HashMap<>();
        for (Zhongdjcfxpgtj item : dataList) {
            String unitName = item.getErjdwmc();
    ...
    public List<SumModel> getRiskAmountOver500w() {
        ...
        for (Zhongdjcfxpgtj item : dataList) {
            ...
            // 大于500W
            if (null != amounFromDB && amounFromDB.compareTo(new BigDecimal(500)) > 0) {
                // 单位名称:审议事项名称+金额
